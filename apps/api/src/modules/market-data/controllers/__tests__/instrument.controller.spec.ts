...
describe("InstrumentController", () => {
  function buildController(overrides: Partial<MarketDataService> = {}) {
    ...
    return { controller: new InstrumentController(marketDataService), marketDataService };
  }

  it("returns a paginated result with correct metadata", async () => {
    const { controller } = buildController();
    const result = await controller.list({ query: "AAPL" } as InstrumentSearchDto, { page: 1, pageSize: 50 } as PaginationQueryDto);
  ...
  it("does not pass exchangeId through to searchInstruments, even if present on the query DTO", async () => {
    ...
    await controller.list({ query: "AAPL", exchangeId: "ex1" } as InstrumentSearchDto, {} as PaginationQueryDto);
  ...
  it("passes assetClass and status through to both searchInstruments and countInstruments consistently", async () => {
    const { controller, marketDataService } = buildController();
    await controller.list({ assetClass: "EQUITY", status: "ACTIVE" } as InstrumentSearchDto, {} as PaginationQueryDto);
    expect(marketDataService.searchInstruments).toHaveBeenCalledWith(
      expect.objectContaining({ assetClass: "EQUITY", status: "ACTIVE" }),
      expect.anything(),
    );
    expect(marketDataService.countInstruments).toHaveBeenCalledWith({ assetClass: "EQUITY", status: "ACTIVE" });
  });

  it("computes hasNextPage correctly across multiple pages", async () => {
    const { controller } = buildController({ countInstruments: jest.fn().mockResolvedValue(120) });
    const result = await controller.list({} as InstrumentSearchDto, { page: 1, pageSize: 50 } as PaginationQueryDto);
