...
import { toPageParams, buildPaginatedResult, PaginatedResult } from "../utils/pagination.util";
...
export class InstrumentController {
  ...
  @ApiOperation({
    ...
    summary: "List/search/filter instruments — query, assetClass, status, page, pageSize.",
    description:
  ...
  async list(
    ...
    const { take, skip, page, pageSize } = toPageParams(pagination);
    // exchangeId deliberately excluded from filters — see this method's
    // @ApiOperation description for why.
    const filters = { assetClass: search.assetClass, status: search.status, search: search.query };
    const [data, totalCount] = await Promise.all([
      this.marketDataService.searchInstruments(filters, { take, skip }),
      this.marketDataService.countInstruments({ assetClass: search.assetClass, status: search.status }),
    ]);
  ...
  @Get(":id")
  @RequirePermissions("market-data.read")
  @ApiOperation({ operationId: "getInstrument", summary: "Get one instrument by id." })
  @ApiOkResponse({ type: InstrumentResponseDto })
  get(@Param("id", ParseUUIDPipe) id: string): Promise<InstrumentResponseDto> {
