...
export class DataQualityIssueRepository {
  ...
  async findByImportJob(importJobId: string, client: DbClient = prisma): Promise<DataQualityIssueModel[]> {
    const rows = await client.dataQualityIssue.findMany({ where: { importJobId }, orderBy: { detectedAt: "asc" } });
    return rows.map(toDataQualityIssueModel);
  }

  /** Quality reports — counts grouped by status, for ValidationReportService and MonitoringController's "Quality Reports" dashboard tile. */
  async countByStatus(client: DbClient = prisma): Promise<Record<string, number>> {
    const groups = await client.dataQualityIssue.groupBy({ by: ["status"], _count: { _all: true } });
