...
export class CouponService {
  ...
  async createCoupon(data: CreateCouponInput, actorId: string, ctx: AuditContext = {}): Promise<Coupon> {
    ...
    await this.eventPublisher.publish(MOD005_EVENTS.COUPON_CREATED, {
      couponId: coupon.id,
      code: coupon.code,
    });
    return coupon;
  ...
  async deactivateCoupon(id: string, actorId: string, ctx: AuditContext = {}): Promise<Coupon> {
    const coupon = await this.couponRepository.deactivate(id);
    await this.auditService.log("billing.coupon.deactivated", {
      userId: actorId,
      entityType: "Coupon",
      entityId: id,
      ...ctx,
  ...
  async applyCoupon(
    ...
    await this.eventPublisher.publish(MOD005_EVENTS.COUPON_REDEEMED, {
