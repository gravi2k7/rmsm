...
export class NotificationWebhookRepository {
  ...
  /** Module 005 addition — the retry processor needs to re-fetch a webhook by id (it only carries the id across the BullMQ job payload, not the full row). */
  findById(id: string, client: DbClient = prisma): Promise<NotificationWebhook | null> {
    return client.notificationWebhook.findUnique({ where: { id } });
  }
