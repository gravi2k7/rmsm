...
export class AdminNotificationController {
  ...
  @RequirePermissions("notification.admin.manage")
  @ApiOperation({
    operationId: "adminGetDeliveryDashboard",
    summary: "Domain 3's Delivery Dashboard — aggregate delivery counts by channel and status across the whole platform.",
  })
  async getDeliveryDashboard(): Promise<{ channel: string; status: string; count: number }[]> {
    return this.deliveryRepository.countByChannelAndStatus();
  }
