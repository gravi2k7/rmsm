...
import { WebhookRetryProcessor } from "./workers/webhook-retry.processor";
import { NotificationCronRegistrar } from "./workers/notification-cron.registrar";
import { WebhookEventBridge } from "./services/webhook-event-bridge.service";
...
import { NotificationCategoryController } from "./notification-category.controller";
...
@Module({
  imports: [
    ...
    BullModule.registerQueue(
      ...
      { name: "digest" },
      { name: "scheduled" },
      // Module 005 Domain 3: "Webhook Retries" — see webhook.service.ts's
      // publishFailureAndScheduleRetry().
      { name: "webhook" },
    ),
  ],
  controllers: [
